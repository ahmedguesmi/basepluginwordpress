 ...
 *
 * @author Guesmi Ahmed (@danielhendricks)
 * @since 0.3.0
 */
...
var renameStrings = [
  [ 'ahmedguesmi\/basepluginwordpress', pkg.config.username + '/' + pkg.name ], // Git/Composer identifier
  ...
  [ 'basepluginwordpress', pkg.name ], // Plugin slug
...
gulp.task( 'rename', () => {
  ...
  return gulp.src( [ './**/*.php', './*.json', './**/*.js', './**/*.scss', './*.txt', './*.md', '!./node_modules/**', '!./vendor/**', '!./.git/**', '!./languages/**', '!./*lock*', '!./gulpfile.js' ] )
    ...
    .pipe( batchRename( function (path) {
      return path.replace( /basepluginwordpress/, pkg.name );
    } ) )
    .pipe( gulp.dest( './' ) )
